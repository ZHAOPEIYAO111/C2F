...
from model import *
# from earlystopping import EarlyStopping
from loss_acc_val_earlystopping import EarlyStopping
from sample import Sampler
import pandas as pd
...
parser.add_argument('--dataset', default="amazon_photo",
                    help="The data set, pubmed, facebook_page, coauthor_cs, coauthor_phy, amazon_photo, amazon_computer")
...
parser.add_argument("--sampling_percent", type=float, default=1,
                    help="The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.")
parser.add_argument("--sampling_percent_1", type=float, default=0.8,
                    help="The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.")
parser.add_argument("--sampling_percent_2", type=float, default=0.5,
                    help="The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.")
parser.add_argument("--judgment_1", type=float, default=1,
                    help="The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.")
parser.add_argument("--judgment_2", type=float, default=0.7,
                    help="The percent of the preserve edges. If it equals 1, no sampling is done on adj matrix.")
...
parser.add_argument("--task_type", default="self-supervised",
...
print(args)
# save log path
log_pth = os.path.join(os.getcwd(), 'logs_fea', method_name, args.dataset)
if os.path.exists(log_pth):
    os.system('rm -r {}'.format(log_pth))  # delete old log, the dir will be automatically built later

model_save_pth = os.path.join(os.getcwd(), 'saved_models_eval', method_name, args.dataset
                              )
...
select_model = SelectModel(save_model_pth=model_save_pth)
...
def train(epoch, train_adj, train_fea, idx_train, idx_val, val_adj=None, val_fea=None, labels=None):
    ...
    train_g = from_networkx(train_g).to('cuda')
    unsupervised_model.cuda()
    classifier_model.cuda()
    ...
    if sampler.learning_type == 'inductive':
        val_g = from_networkx(val_g)
    else:
        val_g = from_networkx(val_g).to('cuda')
    ...
    if args.early_stopping > 0 and sampler.learning_type != "inductive":
        ...
        early_stopping(epoch, acc_val, classifier_model)
        # early_stopping(acc_val, classifier_model)
...
def test(test_adj, test_fea, idx_test, labels):
    ...
    #    test_g = from_networkx(test_g).to('cuda')
    if sampler.learning_type == 'inductive':
        test_g = from_networkx(test_g)
    else:
        test_g = from_networkx(test_g).to('cuda')
...
for epoch in range(args.epochs):
    ...
    best_epoch, classifier_model = select_model(epoch, outputs, classifier_model)
    ...
    # print("best epoch: {}\t loss_test: {:.6f}\t acc_test: {:.10f}\t".format(best_epoch, loss_test, acc_test))
